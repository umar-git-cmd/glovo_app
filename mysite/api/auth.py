...
from mysite.database.schema import UserProfileInputSchema, UserLoginShema
...
async def login(user: UserLoginShema, db: Session = Depends(get_db)):
    ...
    token_db =  RefreshToken(user_id=user_db.id, token=access_token)
    db.add(token_db)
    db.commit()
...
async def logout(refresh_token: str, db: Session = Depends(get_db)):
    ...
    if not stored_token:
        raise HTTPException(status_code=401, detail='неправильная информация')
